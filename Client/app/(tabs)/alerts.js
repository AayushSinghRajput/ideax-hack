...
export default function AlertsScreen() {
  ...
  const [speakingAlertId, setSpeakingAlertId] = useState(null); // track which alert is speaking

  const handleLanguageToggle = () => {
    dispatch(setLanguage(language === "EN" ? "NP" : "EN"));
  };

  const translateText = (enText, npText) => (language === "EN" ? enText : npText);
  ...
  const handlePlayButton = (item) => {
    ...
    const isNowSpeaking = toggleSpeaking(textToSpeak, language);

    setSpeakingAlertId(isNowSpeaking ? item.id : null);
  };
  ...
  const renderAlertItem = ({ item, index }) => {
    ...
    const isPlaying = speakingAlertId === item.id;
                    ...
                    style={[
                      styles.modalIcon,
                      {
                        backgroundColor: getAlertTypeColor(
                          selectedAlert.type || "default"
                        ).icon,
                      },
                    ]}
                  >
                    <Ionicons
                      name={selectedAlert.icon}
                      size={24}
                      color="#FFFFFF"
                    />
                  </View>
                  <Text style={styles.modalAlertTitle}>
                    {translateText(selectedAlert.title, selectedAlert.titleNP)}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#F3F7FF", paddingTop: 15 },
  header: {
  ...
  time: { fontSize: 12, color: "#666", fontWeight: "500", marginLeft: 4 },
  ...
  modalTime: { fontSize: 14, color: "#666", marginLeft: 8, fontWeight: "500" },
  modalDescription: {
    fontSize: 16,
    color: "#333",
    lineHeight: 24,
  ...
  okButtonText: { color: "#FFFFFF", fontSize: 16, fontWeight: "600" },
  playButton: {
  marginLeft: 10,
},
});
